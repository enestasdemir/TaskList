								...
								<c:forEach items="${ ls }" var="item">
									<tr id="${ item.getTaskId() }" role="delete">
										...
										<td><c:if test="${ item.getTaskStatus() == 0}"><span class="badge badge-primary">Active</span></c:if>
											<c:if test="${ item.getTaskStatus() == 1}"><span class="badge badge-warning">Postponed</span></c:if>
											<c:if test="${ item.getTaskStatus() == 2}"><span class="badge badge-danger">Canceled</span></c:if>
											<c:if test="${ item.getTaskStatus() == 3}"><span class="badge badge-success">Finished</span></c:if></td>
										<td><a class="btn btn-primary btn-sm"
											href='<s:url value="/taskedit/${ item.getTaskId() }"></s:url>'
											role="button">Edit</a>
											<button
